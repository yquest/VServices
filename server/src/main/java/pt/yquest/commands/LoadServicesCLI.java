...
import pt.yquest.commands.node.OnCandidates;
import pt.yquest.commands.node.OnElementFound;
import pt.yquest.commands.node.OnNotFound;
...
public class LoadServicesCLI implements AppAction {
    private static final Logger LOGGER = LoggerFactory.getLogger(LoadServicesCLI.class);
    ...
    private Future<Void> loadContent(Promise<Buffer> content) {
        ...
        content.future().onSuccess(buffer -> {
            context.getVertx().sharedData()
                    ...
                    .compute("list", (key, value) -> buffer.toJsonObject());
            result.complete();
        });
